...
class EngineStatus:
    def __init__(self):
        ...
        self.STATUS_TGRAM = f"Pyrofork v{ver.get('pyrotgfork', 'N/A')}"
        self.STATUS_MEGA = f"SDK v{ver.get('mega', 'N/A')}"
...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    for index, task in enumerate(
        ...
        if (
            ...
            msg += (
                f"\n╞ <b>Processed</b> → <i>{task.processed_bytes()}"
                f"{subsize} of {task.size()}</i>"
            )
    ...
    if tasks_no > STATUS_LIMIT:
        msg += (
            f"<b>Page:</b> {page_no}/{pages} | "
            f"<b>Tasks:</b> {tasks_no} | <b>Step:</b> {page_step}\n"
        )
    ...
    msg += (
        f"\n╭╴ <b>CPU</b> → {cpu_percent()}% | "
        f"<b>F</b> → {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)} "
